...
module.exports.createBook = async (req, res) => {
  ...
  if (req.files["image"]) {
    ...
    body.image_url = imageUrl.startsWith("public")
  ...
  if (req.files["book_file"]) {
    let bookFileUrl = req.files["book_file"][0].path;
    body.book_url = bookFileUrl.startsWith("public")
      ? `http://localhost:8000/${bookFileUrl}`
      : bookFileUrl;
  }
